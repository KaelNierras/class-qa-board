import { Card, CardContent, CardHeader } from '@/components/ui/card'
import { Entry } from '@/types';
import React from 'react'

interface EntryCardProps {
    entry: Entry;
    entryType: string;
    setPreviewEntry: React.Dispatch<React.SetStateAction<Entry | null>>;
    index: number;
    total: number;
}
const EntryCard: React.FC<EntryCardProps> = ({ entry, setPreviewEntry, index, total, entryType }) => {
    return (
        <Card
            onClick={() => setPreviewEntry(entry)}
            key={entry.id}
            className="mb-6 rounded-2xl shadow-none border-0 bg-gradient-to-b from-secondary/10 to-primary/10 p-0"
        >
            <CardHeader className="pb-0">
                <div className="text-xs text-primary font-medium mb-2">
                    {entryType === 'question' ? 'Question' : 'Answer'} {index + 1} of {total}
                </div>
                <div className="text-xs mb-2 text-black/60">
                    {entryType === 'question' ? 'Asked by:' : 'Answered by:'} {entry.created_by || 'Unknown'}
                </div>
            </CardHeader>
            <CardContent className="pt-0 pb-6">
                <div className="text-xl font-semibold text-black leading-snug mb-2">
                    {entry.text}
                </div>
            </CardContent>
        </Card>
    )
}

export default EntryCard